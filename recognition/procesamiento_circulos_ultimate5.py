# RECONOCIMIENTO DE CIRCULOS GRADO 1 y 2
...
def detectar_respuestas_estudiante(ruta_imagen, formato_columnas, dp_values=[1.002, 1.003, 1.004, 1.005, 1.007, 1.008, 1.009, 1.010, 1.012, 1.013, 1.014, 1, 0.999, 0.998, 0.997, 0.85]):
    ...
    total_preguntas_esperadas = sum(formato_columnas)

    for dp in dp_values:
        print(f"Probando con dp = {dp}")
        roi_dp_color = roi.copy()

        circulos = cv2.HoughCircles(roi_gray, 
                                    cv2.HOUGH_GRADIENT, 
                                    dp=dp, # Config para segundo comentada
                                    minDist=5, # 2 -> 5
                                    param1=40, # 2 -> 40
                                    param2=17, # 2 -> 17
                                    minRadius=9, # 2 -> 7
        ...
        filas_ref = max(filas_columnas, key=len)
        ...
        for idx_col, filas in enumerate(filas_columnas):
            ...
            for fila in alineadas:
                if pregunta > total_preguntas_esperadas:
                    break
    ...
    for respuestas in respuestas_por_dp:
        for pregunta, respuesta in respuestas.items():
            if int(pregunta) <= total_preguntas_esperadas and (
                pregunta not in respuestas_combinadas or respuestas_combinadas[pregunta] == "No marcada"
            ):
    ...
    if cantidad_por_dp:
        idx_mejor = cantidad_por_dp.index(max(cantidad_por_dp))
        imagen_mejor = visualizaciones_por_dp[idx_mejor]
        #plot_image(imagen_mejor, "Detección más efectiva", grayscale=False)

    return dict(sorted(respuestas_combinadas.items(), key=lambda x: int(x[0])))

# Prueba
if __name__ == "__main__":
    ruta = "C:/Users/valde/Desktop/image-recognition/assets/GRADO 101/202504081543_00029.jpg"
    formato = [15, 15, 15, 13]
